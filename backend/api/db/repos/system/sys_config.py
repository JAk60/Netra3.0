...
class SystemConfigurationRepository:
    ...
    def _register_equipment_sync(
            ...
            try:
                # 1️⃣ SYSTEM (use system_type from request, create if not exists)
                system = session.exec(
                    select(System).where(System.system_type == data.system_type)
                ).first()

                if not system:
                    system = System(system_type=data.system_type)
                    session.add(system)
                    session.flush()
                ...
                # 3️⃣ DEPARTMENT (scoped to ship, get or create)
                ...
                # 🚨 4️⃣ UNIQUENESS CHECK (CMMS + NOMENCLATURE + SHIP)
                ...
                logger.info(
                    ...
                    f"System: {data.system_type} | Ship: {ship.ship_name} | Dept: {department.department_name}"
                )
    ...
    def _get_hierarchy_by_component_id_and_ship_id_sync(
        self, session: Session, component_id: UUID, ship_id: UUID
    ) -> Dict[str, Any]:
        """Synchronous: get complete hierarchy for a component by component_id AND ship_id"""
        try:
            # First verify ship exists
            ship = session.get(Ship, ship_id)
            if not ship:
                logger.error(f"Ship with ID '{ship_id}' not found")
                return {}

            # Ensure the component belongs to a department in this ship
            component_statement = (
                select(SystemConfiguration)
                .join(Department, Department.department_id == SystemConfiguration.department_id)
                .where(
            ...
            if not component:
                logger.error(
                    f"Component '{component_id}' not found in ship '{ship.ship_name}'"
                )
                return {}

            # Recursive children builder
            def get_descendants(comp_id: UUID) -> List[Dict]:
                children = self._get_children_sync(session, comp_id)
                result = []
                for child in children:
                    result.append({
            ...
            logger.error(
                f"Failed to get hierarchy for component '{component_id}' in ship '{ship_id}': {e}"
            )
            raise

    async def get_hierarchy_by_component_id_and_ship_id(
        self, component_id: UUID, ship_id: UUID
    ) -> Dict[str, Any]:
        """Async wrapper for the sync hierarchy fetch"""
        
        def _run():
            with get_session_context() as session:
                return self._get_hierarchy_by_component_id_and_ship_id_sync(
                    session, component_id, ship_id
                )
        ...
        try:
            ...
            statement = (
                ...
                .join(
                    SystemConfiguration, 
                    and_(
                        Ship.ship_id == SystemConfiguration.ship_id,
                        SystemConfiguration.parent_id.is_(None)
                    ),
                    isouter=True
                )
