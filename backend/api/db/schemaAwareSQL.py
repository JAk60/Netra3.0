...
def test_simple_query():
    """Enhanced test function with multiple test cases"""
    ...
    test_queries = [
        "List all sensors on ship INS ONE",
        "what is the alpha value of GT 1",
        "Show all sensors on nomenclature GT 1"
    ]
    
    results = {}
    
    for test_question in test_queries:
        logger.info(f"🧪 Testing query: {test_question}")
        
        try:
            result = process_query(test_question, user_identity)
            results[test_question] = result
            
            if isinstance(result, dict) and "error" in result:
                logger.error(f"❌ Test failed for '{test_question}': {result}")
            else:
                logger.info(f"✅ Test passed for '{test_question}': {len(result.get('result', []))} records")
                
        except Exception as e:
            logger.error(f"❌ Test exception for '{test_question}': {e}")
            results[test_question] = {"error": "Test Exception", "message": str(e)}
...
if __name__ == "__main__":
    ...
    try:
        initialize()
        test_results = test_simple_query()
        print("=== Test Results ===")
        for query, result in test_results.items():
            ...
            if isinstance(result, dict) and "error" in result:
                ...
                print(f"✅ Success: {result.get('records_retrieved', 0)} records")
    except Exception as e:
