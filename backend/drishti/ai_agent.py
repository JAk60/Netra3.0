...
from utils.utils import get_llm_client, load_config
...
class AIAgent:
    def __init__(self):
        ...
        logger.info(
            f"📌 Initialized memory: {'Unlimited' if max_memory_length == 0 else f'Last {max_memory_length} exchanges'}")
    ...
    def chat(self, user_input, user_identity):
        ...
        past_chat = self.memory.load_memory_variables(
            {}).get("chat_history", [])
