...
class LLMClient:
    def __init__(self, config):
        ...
        self.provider = config.get(
            "LLM_PROVIDER", "").strip().lower()  # Normalize value
        ...
        try:
            if self.provider == "openai":
                ...
                logger.info(
                    f"📌 Using OpenRouter model: {self.model}, API URL: {self.base_url}")
    ...
    def llm_client_validate_sql(self, sql_query):
        ...
        response = "PLACEHOLDER: call your validation model here. it can be the same model or a different one"
        ...
        try:
            ...
            if self.provider == "openai":
                if not hasattr(self, 'client') or self.client is None:
                    ...
                    return {"error": "Client Error", "message": "OpenAI client is not properly initialized"}
                
                response_text = self.client.invoke(prompt).content

            elif self.provider == "llama":
                generator = pipeline("text-generation", model=self.model)
                response_text = generator(prompt, max_length=256)[
                    0]["generated_text"]

            elif self.provider == "llama_local":
                if not hasattr(self, 'client') or self.client is None:
                    logger.error("❌ Llama local client is not initialized")
                    return {"error": "Client Error", "message": "Llama local client is not properly initialized"}
                
                response_text = self.client(prompt, temperature=0)[
                    "choices"][0]["text"]
                ...
                logger.error(f"❌ Unsupported LLM Provider: {self.provider}")
                return {"error": "Unsupported Provider", "message": f"Unsupported LLM Provider: {self.provider}"}
    ...
    def extract_sql(response_text):
        ...
        # Log first 200 chars
        logger.debug(
            f"👉 Extracting SQL from response: {response_text[:200]}...")
        ...
        match = re.search(r"```sql(.*?)```", response_text,
                          re.DOTALL)  # Extract SQL inside ```sql ```
        ...
        match = re.search(r"\bSELECT\b\s+.*", response_text,
        ...
        # Handle known error messages - return as dictionary for proper handling
        ...
        if "i don't know" in lower_text:
            logger.warning(
                "❌ LLM response indicates it will not answer questions outside the topic of connected database.")
        ...
        if "clarify:" in lower_text:
            logger.warning(
                "❌ LLM response requests clarification from the user.")
