...
class RCMTool(BaseTool):
    ...
    def _get_cache_key(self, name: Union[str, List[str]], ships: Optional[List[str]]) -> str:
        """Generate cache key for query"""
        name_str = ",".join(sorted(name)) if isinstance(name, list) else name
    ...
    def _normalize_name(self, name: Union[str, List[str]]) -> Union[str, List[str]]:
        ...
        Handles edge cases like empty strings, whitespace, etc.
        """
    ...
    def _build_description(
        ...
        if len(formatted_results) == 1:
            ...
            if result.get("has_rcm"):
                ...
                return (
                    f"Found RCM record for {result['nomenclature']}{ship_info}. "
                    f"Maintenance Policy: {policy_preview}"
                )
            else:
                return f"No RCM record found for {result['nomenclature']}{ship_info}."
        else:
            ...
            return (
                ...
                f"({summary['unique_nomenclatures']} nomenclature(s){ship_info})"
            )
