...
class ShipComponentQuerySystem:
    ...
    def is_system_type_match(self, system_type: str, requested_components: List[str]) -> bool:
        """Check if system type matches any requested component with better matching logic"""
        if not system_type or not requested_components:
            return False
        ...
        for requested in requested_components:
            ...
            # Exact match
            if system_type_lower == requested_lower:
                return True
            
            # Check if requested component is contained in system type
            # e.g., "propulsion" matches "propulsion system"
            if requested_lower in system_type_lower:
                return True
            
            # Check if system type is contained in requested component
            # e.g., "propulsion system" matches "propulsion"
            if system_type_lower in requested_lower:
                return True
    ...
    def should_include_system(self, system: Dict, requested_components: List[str]) -> bool:
        ...
        # Check if system type is requested - use improved matching
        system_type = system.get('system_type', '')
        if self.is_system_type_match(system_type, requested_components):
            return True
        
        # OPTION 1: Comment out this section if you want ONLY system-level filtering
        # This prevents cross-system component matching
        """
        ...
        # OPTION 2: More restrictive component matching
        # Only include if components explicitly belong to the requested system type
        system_component_ids = system.get('has_components', [])
        for comp_id in system_component_ids:
            if comp_id in requested_components:
                # Additional check: only include if the component logically belongs to this system
    ...
    def should_include_component(self, component: Dict, requested_components: List[str]) -> bool:
        ...
        # Check exact component ID match
        if component_id in requested_components:
            return True
        
        # Check component type match
        for requested in requested_components:
            if requested.lower() in component_type.lower() or component_type.lower() in requested.lower():
                return True
    ...
    def filter_system_preserve_structure(self, system: Dict, requested_components: List[str]) -> Optional[Dict]:
        ...
        for component in components:
            # More restrictive filtering logic
            should_include = False
            
            # If no specific components requested, include all from this system
            if not requested_components:
                should_include = True
            # If this system type was requested, include all its components
            elif self.is_system_type_match(system.get('system_type', ''), requested_components):
                should_include = True
            # If this specific component was requested
            elif self.should_include_component(component, requested_components):
                should_include = True
    ...
    async def process_natural_language_query(self, message: str) -> Dict[str, Any]:
        ...
        print(f"DEBUG: Extracted components: {requested_components}")
