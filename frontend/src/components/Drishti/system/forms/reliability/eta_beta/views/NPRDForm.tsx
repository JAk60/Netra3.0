...
export const NPRDForm: React.FC<NPRDFormProps> = ({
  ...
  useEffect(() => {
    const points = getNPRDData(selectedAssembly);
    setDataPoints(points);
  }, [selectedAssembly, getNPRDData]);
  ...
  const onSubmit = async (data: NPRDData) => {
    ...
    if (editingId) {
      ...
      updateNPRDData(selectedAssembly, editingId, entry);
      ...
      addNPRDData(selectedAssembly, entry);
      toast.success('NPRD data added successfully!', {
  ...
  const handleSubmitToServer = async () => {
    if (dataPoints.length === 0) {
      toast.error('No data to submit', {
        description: 'Please add some NPRD data first',
      });
      return;
    }
    ...
    try {
      ...
      // await apiClient.post('/api/eta-beta/nprd-data', {
      //   assemblyId: selectedAssembly,
      //   dataPoints: dataPoints
      // });

      console.log('Submitting NPRD Data:', dataPoints);

      toast.success('NPRD data submitted successfully!', {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between text-lg">
            <span>
              {editingId ? 'Edit' : 'Add'} NPRD (Navy Parts Reliability Data) - {assemblyLabel}
            </span>
            <Button
              type="button"
              variant="ghost"
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {editingId ? 'Update NPRD Data' : 'Add NPRD Data'}
              </Button>
              ...
              <Button 
                type="button" 
                variant="outline" 
                onClick={() => reset({ failureRate: 2, beta: 2 })}
                disabled={isSubmitting}
              >
                Reset
              </Button>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">
                Existing NPRD Data ({dataPoints.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
                  ...
                  >
                    <div className="flex-1 grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-medium">{point.failureRate}</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Beta (β)</p>
                        <p className="font-medium">{point.beta}</p>
                      </div>
                    </div>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    Submit {dataPoints.length} NPRD Data Point{dataPoints.length > 1 ? 's' : ''} to Server
                  </>
                )}
              </Button>
            </CardContent>
