...
export const OEMExpertForm: React.FC<Props> = ({ selectedAssembly, assemblyLabel }) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await createOEMExpertData({ component_id: selectedAssembly, ...data });
      toast.success('OEM + Expert data saved!', { description: `Saved for ${assemblyLabel}` });
      reset();
    } catch (error) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">OEM + Expert Judgement — {assemblyLabel}</CardTitle>
      </CardHeader>
        ...
        <div className="flex gap-2">
          <Button onClick={handleSubmit(onSubmit)} disabled={isSubmitting} className="flex-1">
            {isSubmitting
              ...
              : <><Send className="w-4 h-4 mr-2" />Save OEM + Expert Data</>
            }
          </Button>
