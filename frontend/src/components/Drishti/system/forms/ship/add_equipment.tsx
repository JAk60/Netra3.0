...
import { createComponent } from '@/actions/createEquipment';
...
export default function AddComponentForm({ systems, components, departments, loading, shipId }: ManageComponentsProps) {
  ...
  const selectedSystemType = watch('systemType');
  const selectedSystem = systems.find(s => s.type === selectedSystemType);
  ...
  const onSubmit = async (data: ComponentFormData) => {
    ...
    try {
      ...
      const result = await createComponent(data, shipId, selectedSystem.id);
